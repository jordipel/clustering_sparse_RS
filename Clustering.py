...
def getMethodData(day):

    # Cargar los datos
    ADS_RAW = load_sparse_csr("SPARSES/SPARSE_CASOS_USO_DAY" + str(day))
    # USR_RAW = load_sparse_csr("SPARSES/SPARSE_USUARIOS_DAY"+str(day))
    ...
    ADS_RAW = ADS_RAW[CUANTOS:, 667:]  # TEST

    USR_RAW = load_sparse_csr("SPARSES/SPARSE_USUARIOS_DAY"+str(day))

    h5f = h5py.File('VALORACIONES/VAL_POR_CENTROIDE_RAW_TRAIN_DAY'+str(day)+'.h5', 'r')
    VAL_TRAIN = h5f['VAL_POR_CENTROIDE'][:]
    h5f.close()

    h5f = h5py.File('VALORACIONES/VAL_POR_CENTROIDE_RAW_TEST_DAY'+str(day)+'.h5', 'r')
...
def performTestT(IM1, IM2, verbose=False):
    ...
    IM1 = IM1.reshape((1, K * K))[0]
    IM2 = IM2.reshape((1, K * K))[0]
...
def getDIFFcentroids(K=100, day=1, SEED=100, verbose=1, testData=False):
    # Calcula centroides de diferencias de anuncios

    # Cargar los datos
    ADS = load_sparse_csr("SPARSES/SPARSE_CASOS_USO_DAY"+str(day))
    ...
    if testData:
        FILENAME = "DIFF_RAW_CENTROIDS_TEST_DAY" + str(day)
        ADS_RAW = ADS[CUANTOS:, 667:]  # TEST
    else:
        FILENAME = "DIFF_RAW_CENTROIDS_TRAIN_DAY" + str(day)
        ADS_RAW = ADS[0:CUANTOS, 667:]  # TRAIN

    # Clustering de diferencias de anuncios
    # seedingDIFF_RAW = kmc2.kmc2(ADS_RAW, K, afkmc2=True, random_state=SEED)  # Run k-MC2
    # modelDIFF_RAW = MiniBatchKMeans(K, init=seedingDIFF_RAW, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
    modelDIFF_RAW = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024, verbose=verbose,
                                    random_state=SEED)
    modelDIFF_RAW.fit(ADS_RAW)

    toPickle(FILENAME, modelDIFF_RAW.cluster_centers_)
    toPickle(FILENAME.replace("CENTROIDS", "LABELS"), modelDIFF_RAW.labels_)


def methodTwo(K=100, SEED=100, verbose=1, day=1, loadData=False):
    ...
    USR_RAW, DIFF_RAW,  VAL_TRAIN, VAL_TEST = getMethodData(day)
    ...
    if not loadData:
        # los comento porque no tengo instalado kmc2
        # seedingUSR_RAW = kmc2.kmc2(USR_RAW, K, afkmc2=True, random_state=SEED)  # Run k-MC2
        # seedingDIFF_RAW = kmc2.kmc2(DIFF_RAW, K, afkmc2=True, random_state=SEED)  # Run k-MC2

        print("Centroides generados...")

        # modelUSR_RAW = MiniBatchKMeans(K, init=seedingUSR_RAW, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
        # modelDIFF_RAW = MiniBatchKMeans(K, init=seedingDIFF_RAW, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
        modelUSR_RAW = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                       verbose=verbose, random_state=SEED)
        modelDIFF_RAW = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                        verbose=verbose, random_state=SEED)

        modelUSR_RAW.fit(USR_RAW)
        # esto creo que ya se hizo y está almacenado en DIFF_RAW_LABELS_TEST
        # modelDIFF_RAW.fit(DIFF_RAW)

        model_USR_RAW_labels = modelUSR_RAW.labels_
        # model_DIFF_RAW_labels = modelDIFF_RAW.labels_
        model_DIFF_RAW_labels = getPickle("DIFFS/DIFF_RAW_LABELS_TEST_" + DAY)

        toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_USR_RAW_labels", model_USR_RAW_labels)
        # toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels", model_DIFF_RAW_labels)

    else:

        model_USR_RAW_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_USR_RAW_labels")
        # model_DIFF_RAW_labels = getPickle("ClusteringResults/model_" + DAY + "_" + str(K) + "_DIFF_RAW_labels")
        model_DIFF_RAW_labels = getPickle("DIFFS/DIFF_RAW_LABELS_TEST_" + DAY)

    #####################################################################

    print("[" + str(K) + " Clusters] Usuarios RAW , Anuncios RAW")
    print("U_raw DIA:", day)
    IM1 = getIncertidumbre(K, VAL_TEST, model_USR_RAW_labels, model_DIFF_RAW_labels,
                           fileName="ClusteringResults/Inc/METHOD2_DAY"+str(day)+"_RAW_INC")
    print("-" * 65)


def methodThree(K=100, SEED=100, verbose=1, day=1, loadData=False):
    ...
    if not loadData:
        # los comento porque no tengo instalado kmc2
        # seedingUSR_RAW_VAL = kmc2.kmc2(USR, K, afkmc2=True, random_state=SEED)  # Run k-MC2
        # seedingDIFF_RAW = kmc2.kmc2(DIFF_RAW, K, afkmc2=True, random_state=SEED)  # Run k-MC2

        print("Centroides generados...")

        # modelUSR_RAW_VAL = MiniBatchKMeans(K,init=seedingUSR_RAW_VAL, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
        # modelDIFF_RAW = MiniBatchKMeans(K,init=seedingDIFF_RAW, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
        modelUSR_RAW_VAL = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                           verbose=verbose, random_state=SEED)
        modelDIFF_RAW = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                        verbose=verbose, random_state=SEED)
        ...
        toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_USR_RAW_VAL_labels", model_USR_RAW_VAL_labels)
        # toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels", model_DIFF_RAW_labels)

    else:
        model_USR_RAW_VAL_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_USR_RAW_VAL_labels")
        # model_DIFF_RAW_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels")
        model_DIFF_RAW_labels = getPickle("DIFFS/DIFF_RAW_LABELS_TEST_" + DAY)

    #####################################################################

    print("[" + str(K) + " Clusters] Usuarios RAW + valoración RAW (%), Anuncios RAW")
    print("U_raw_uti DIA:", day)
    getIncertidumbre(K, VAL_TEST, model_USR_RAW_VAL_labels, model_DIFF_RAW_labels,
                     fileName="ClusteringResults/Inc/METHOD3_DAY"+str(day)+"_RAW_INC")
    print("-" * 65)


def methodFour(K=100, SEED=100, verbose=1, day=1, loadData=False):
    ...
    if not loadData:
        # los comento porque no tengo instalado kmc2
        # seedingVAL_RAW = kmc2.kmc2(VAL_TRAIN, K, afkmc2=True, random_state=SEED)  # Run k-MC2
        # seedingDIFF_RAW = kmc2.kmc2(DIFF_RAW, K, afkmc2=True, random_state=SEED)  # Run k-MC2

        print("Centroides generados...")

        # modelVAL_RAW = MiniBatchKMeans(K, init=seedingVAL_RAW, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
        # modelDIFF_RAW = MiniBatchKMeans(K, init=seedingDIFF_RAW, max_no_improvement=1000, batch_size=1024, verbose=verbose,random_state=SEED)
        modelVAL_RAW = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                       verbose=verbose, random_state=SEED)
        modelDIFF_RAW = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                        verbose=verbose, random_state=SEED)
        ...
        toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_VAL_RAW_labels", model_VAL_RAW_labels)
        # toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels", model_DIFF_RAW_labels)

    else:
        model_VAL_RAW_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_VAL_RAW_labels")
        # model_DIFF_RAW_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels")
        model_DIFF_RAW_labels = getPickle("DIFFS/DIFF_RAW_LABELS_TEST_" + DAY)

    #####################################################################

    print("[" + str(K) + " Clusters] Valoración RAW (%), Anuncios RAW")
    print("U_uti DIA:", day)
    getIncertidumbre(K, VAL_TEST, model_VAL_RAW_labels, model_DIFF_RAW_labels,
                     fileName="ClusteringResults/Inc/METHOD4_DAY"+str(day)+"_VAL_INC")
    print("=" * 65)


def methodFourDotTwo(K=100, SEED=100, verbose=1, day=1, loadData=False):

    DAY = "DAY" + str(day)

    USR_RAW, DIFF_RAW,  VAL_TRAIN, VAL_TEST = getMethodData(day)

    CLUSTER_SIZE = getPickle("DIFFS/DIFF_RAW_LABELS_TRAIN_"+DAY)
    ...
    if not loadData:
        ...
        toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_VALPOND_RAW_labels", model_VAL_RAW_labels)
        # toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels", model_DIFF_RAW_labels)

    else:
        model_VAL_RAW_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_VALPOND_RAW_labels")
        # model_DIFF_RAW_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_DIFF_RAW_labels")
        model_DIFF_RAW_labels = getPickle("DIFFS/DIFF_RAW_LABELS_TEST_" + DAY)
    ...
    print("[" + str(K) + " Clusters] Valoración RAW (%), Anuncios RAW")
    print("U_w_uti DIA:", day)
    getIncertidumbre(K, VAL_TEST, model_VAL_RAW_labels, model_DIFF_RAW_labels,
                     fileName="ClusteringResults/Inc/METHOD4.2_DAY"+str(day)+"_VALPOND_INC")
    print("=" * 65)


def methodEMB(K=100, SEED=100, verbose=1, day=1, loadData=False):
    # Cluster con los usuarios en crudo
    ...
    h5f = h5py.File('EMBEDDINGS/EMBEDDING_USUARIOS_DAY' + str(day) + '.h5', 'r')
    ...
    if not loadData:
        ...
        modelUSR_EMB = MiniBatchKMeans(K, init='k-means++', max_no_improvement=1000, batch_size=1024,
                                       verbose=verbose, random_state=SEED)
        ...
        toPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_USR_EMB_labels", model_USR_EMB_labels)

    else:

        model_USR_EMB_labels = getPickle("ClusteringResults/model_"+DAY+"_"+str(K)+"_USR_EMB_labels")
        model_DIFF_RAW_labels = getPickle("DIFFS/DIFF_RAW_LABELS_TEST_" + DAY)

    #####################################################################

    print("[" + str(K) + " Clusters] Usuarios EMB , Anuncios RAW")
    print("U_emb DIA:", day)
    IM1 = getIncertidumbre(K, VAL_TEST, model_USR_EMB_labels, model_DIFF_RAW_labels,
                           fileName="ClusteringResults/Inc/METHOD2_DAY"+str(day)+"_RAW_INC")
    print("-" * 65)
...
for i in range(DAY, DAY+1):
    DAY = i
    printW("Dia "+str(i))
    # getDIFFcentroids(K, DAY, SEED=100, verbose=1, testData=True)  # para obtener ficheros DIFF_* con los clusters
    methodTwo(K=K, day=DAY, loadData=CARGA)  # En el artículo: Cl(Uraw)
    methodThree(K=K, day=DAY, loadData=CARGA)  # En el artículo: Cl(Uraw + Uti)
    methodFour(K=K, day=DAY, loadData=CARGA)  # En el artículo: Cl(U Uti)
    methodFourDotTwo(K=K, day=DAY, loadData=CARGA)  # En el artículo: Cl(UW-Uti)
    methodEMB(K=K, day=DAY, loadData=CARGA)  # En el artículo: Cl(Uemb)
...
'''
h5f = h5py.File('hdf/EMBEDDING_USUARIOS.h5','r')
XU = h5f['embedding'][:]
h5f.close()

h5f = h5py.File('hdf/EMBEDDING_ANUNCIOS.h5','r')
XA = h5f['embedding'][:]
h5f.close()

KMU, KMA = getClustersMatrix(XU,XA,nclust=10,nitmax=100,verbose=1, seed=10, doPlot=True)

exit()

stt = time.time()

h5f = h5py.File('hdf/EMBEDDINGS.h5','r')
XU = h5f['kk'][0:10000]
h5f.close()

print(time.time()-stt)

XA = load_sparse_csr("SPARSE_ANUNCIOS")
MT = getClustersMatrix(XU,XA,init="random",nclust=10,nitmax=100,verbose=1, seed=10 )
exit()


# GENERAR CSV CON MATRIZ EN TENSOR [RAW]
#-----------------------------------------------------------------------

fileObject = open("ClusteringTensor/grupos_CRUDO_U.p",'r')
CLU = pickle.load(fileObject)

fileObject = open("ClusteringTensor/grupos_CRUDO_A.p",'r')
CLA = pickle.load(fileObject)

nclust = max(CLU)+1
matName = "ClusteringTensor/Output/KM_"+str(nclust)+"_TF_"

RET = getMatrix(nclust, CLU, CLA, plot=True,invert=False, percent=False, fileName=matName + "N",entropy=True)
RET_P = getMatrix(nclust, CLU, CLA, plot=True,invert=False, percent=True, fileName=matName + "P")
RET_P_T = getMatrix(nclust, CLU, CLA, plot=True,invert=True, percent=True, fileName=matName + "P_T")

exit()

'''
'''
# GENERAR CSV CON MATRIZ EN TENSOR [EMB]
#-----------------------------------------------------------------------

fileObject = open("ClusteringTensor/grupos_EMB_U.p",'r')
CLU = pickle.load(fileObject)

fileObject = open("ClusteringTensor/grupos_EMB_A.p",'r')
CLA = pickle.load(fileObject)
...
matName = "ClusteringTensor/Output/KM_"+str(nclust)+"_TF_EMB_"
...
# GENERAR CSV CON EL RESUMEN DE LO QUE CONTIENE LOS CLUSTERS DE USUARIOS
#-----------------------------------------------------------------------
fileObject = open("ClusteringTensor/grupos_CRUDO_U.p",'r')
TENSOR_LABELS = pickle.load(fileObject)

fileObject = open("ClusteringTensor/grupos_EMB_U.p",'r')
TENSOR_EMB_LABELS = pickle.load(fileObject)

XU = load_sparse_csr("SPARSE_USUARIOS")

getUsersClusterInfo(XU, TENSOR_LABELS, filename="ClusterUserInfo_TENSOR")
getUsersClusterInfo(XU, TENSOR_EMB_LABELS, filename="ClusterUserInfo_TENSOR_EMB")


getIncertidumbre(XU,TENSOR_EMB_LABELS)


exit()

# GENERAR CSV CON MATRIZ EN SCIKIT [RAW]
#-----------------------------------------------------------------------
XU = load_sparse_csr("SPARSE_USUARIOS")
XA = load_sparse_csr("SPARSE_ANUNCIOS")

KMU, KMA = getClustersMatrix(XU,XA,nclust=100,nitmax=100,verbose=0, seed=10, doPlot=True)

exit()

# GENERAR CSV CON MATRIZ EN SCIKIT [EMB]
#-----------------------------------------------------------------------
...
KMU, KMA = getClustersMatrix(XU,XA,nclust=10,nitmax=100,verbose=0, seed=10, doPlot=True)
